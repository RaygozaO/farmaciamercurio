...
function fetchDomicilio(codigoPostal) {
	if (codigoPostal.length === 5 && codigoPostal >= 20000) { // Verifica que el código postal tenga 5 dígitos y sea válido
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "./models/usuarioModel.php", true);
		xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		xhr.onreadystatechange = function () {
			if (xhr.readyState === 4 && xhr.status === 200) {
				var response = JSON.parse(xhr.responseText);
				if (response.success) {
					document.getElementById('colonia').value = response.data.nombrecolonia;
					document.getElementById('municipio').value = response.data.nombremunicipio;
					document.getElementById('entidad').value = response.data.nombreentidad;
				} else {
					console.log('Error: ' + response.message);
				}
			}
		};
		xhr.send("codigo_postal=" + codigoPostal);
	} else {
		// Limpiar campos si el código postal es inválido
